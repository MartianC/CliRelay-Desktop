...
**当前结论：** Task 1 到 Task 4 已完成。上游 CliRelay binary、`config.example.yaml` 和 codeProxy panel dist 不进入 git；它们由 `pnpm upstream:fetch` 按 `upstream-lock.json` 下载、校验和放置，并由 `.gitignore` 忽略。

**下一步：** 从 Task 5 开始实现路径、设置和默认配置生成。阶段 A 目前只完成工程骨架和上游供应链脚本；运行时 Sidecar 启动、panel 复制、External/Owned 判定和退出清理仍在后续 Task 中完成。
...
Expected: `pnpm test` 通过 6 个用例；`pnpm upstream:verify` 通过；`git ls-files ...` 无输出；`git check-ignore -v ...` 命中 `.gitignore` 中的上游 fetch 输出规则。
