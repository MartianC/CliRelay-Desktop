...
- [ ] Task 1：初始化 Tauri 2 + React + TypeScript 项目骨架
- [ ] Task 2：写入项目元数据、License、README 和非官方声明
...
- [ ] **Step 4: 添加 MIT License 和 notices 初始内容**
...
- Create: `src-tauri/resources/config.example.yaml`
- Create: `src-tauri/resources/panel/manage.html`
- Create: `src-tauri/binaries/clirelay-aarch64-apple-darwin`

**代码改动原因：** 构建和发布不能依赖手工复制 Sidecar 或运行时 GitHub 下载。脚本必须从 lock 下载、计算 SHA-256、解包 CliRelay 二进制、复制 codeProxy panel 资源、重命名为 Tauri sidecar 目标文件，并复制上游 config example 作为默认配置来源。
...
Expected: 脚本不会修改 `upstream-lock.json`。
...
- [ ] **Step 5: 下载真实 Sidecar 并提交**

```bash
pnpm upstream:fetch
pnpm upstream:verify
git add scripts tests package.json pnpm-lock.yaml upstream-lock.json src-tauri/tauri.conf.json src-tauri/binaries src-tauri/resources
git commit -m "feat: fetch verified upstream assets"
```

Expected: `src-tauri/binaries/clirelay-aarch64-apple-darwin` 存在且可执行，`src-tauri/resources/panel/manage.html` 存在。
...
```bash
cargo test --manifest-path src-tauri/Cargo.toml settings paths
```
